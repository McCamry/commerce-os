...
### Added
- Web i18n and theming foundation. A custom `I18nProvider` (Thai default, English switch) with flat dot-namespaced dictionaries, `useT()`, and `{var}` interpolation; every label and combo-box option now goes through `t()`. Organization-specific overrides are fetched from `GET /translations` and merged over the base dictionaries, editable at Settings → Translations. A custom `ThemeProvider` (light/dark/system, persisted, no-flash) with a sidebar switcher, built on the existing `.dark` CSS variables. Retrofitted Dashboard, Products, Customers, Sales Orders, Inventory, and Login. See `docs/UI_GUIDELINE.md` for the required pattern for future pages.
